...
def run_linear(
    ...
    linear = Linear(d_in, d_out, weights, device, torch.float32)
...
def run_swiglu(
    ...
    ffn = FFN(d_model, d_ff, w1_weight, w2_weight, w3_weight)
    return ffn.SwiGLU(in_features)
...
def run_transformer_lm(
    ...
    LLM = TransformerLM(vocab_size, context_length, d_model, num_layers, num_heads, d_ff, rope_theta, weights)
...
def run_rmsnorm(
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    rmsnorm = RMSNorm(d_model, eps, device=device, dtype=torch.float32)
    rmsnorm.set_gain(weights)
...
def run_get_batch(
    ...
    """
    from cs336_basics.training import data_loading
...
def run_cross_entropy(
    ...
    return closs.loss(inputs, targets)
...
def get_tokenizer(
    ...
    """
    from cs336_basics.bpe_tokenizer import BPETokenizer, BPETokenizer_example
    # use the implementation in slides, warning: failed
    # return BPETokenizer_example(vocab, merges, special_tokens)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    """
    #raise NotImplementedError
    from cs336_basics.bpe_tokenizer_training import train_bpe, train_bpe_example
    # use the implementation in slides, warning: timeout and failed
    # bpe_params = train_bpe_example(input_path, vocab_size, special_tokens)
