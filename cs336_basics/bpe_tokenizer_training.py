...
from cs336_basics.bpe_tokenizer import BPETokenizerParams, merge
...
def pre_tokenization_chunk(chunk_string, special_tokens: list[str] = None) -> dict[tuple[bytes], int]:
    # Removing special tokens before pre-tokenization
    if("" in special_tokens): special_tokens.remove("")
    if(special_tokens != None):
        split_chunks = re.split('|'.join(special_tokens), chunk_string)
    else:
        split_chunks = [chunk_string]
...
def pre_tokenization_corpus(corpus_path: str, special_tokens: list[str] = None, num_processes: int = 4) -> dict[tuple[bytes], int]:
    ...
    with open(corpus_path, "rb") as file:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            file.seek(start)
            chunk_string: str = file.read(end - start).decode("utf-8", errors="ignore")
...
def train_bpe(input_path: str, vocab_size: int, special_tokens: list[str]) -> BPETokenizerParams:
    ...
    """
    frequency_table = pre_tokenization_corpus(input_path, special_tokens, os.cpu_count()) # dict[tuple[bytes], int]
    # init
...
def main():
    ...
    train_bpe(os.path.join(dir_path, "../", "tests/fixtures", "corpus.en"), 500, special_tokens=["<|endoftext|>"])
